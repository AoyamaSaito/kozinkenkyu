...
CLUES: list[Clue] = [
    ...
    Clue(
        "p1_02", "先遣隊 活動記録", 1,
        "[20██年5月12日] 第3日。B区画の調査を継続。設備の劣化が激しいが通行に支障なし。明日は奥の区画に入る予定。──以降の記録はない。",
    ),
    Clue(
        "p1_03", "防護服のロッカー", 1,
    ...
    Clue(
        "p1_06", "収容監視ログ", 1,
        "[自動記録] 20██/5/8 収容状態：正常 20██/5/9 収容状態：正常 20██/5/10 収容状態：正常 20██/5/11 収容状態：正常 20██/5/12 収容状態：異常 ──Loss detected── 以降の記録なし。",
    ),
    Clue(
        "p1_07", "特殊保管容器（空）", 1,
    ...
    Clue(
        ...
        "管理番号: OBS-0041 [19██年██月██日] 定期観察記録。被験者7名中、5名に認知機能低下を確認。2名は異常なし。記録と一致せず。2系統の反応パターンを観測。要精査。",
        professor_extra=(
    ...
    Clue(
        "p4_04", "封鎖プロトコル通知", 4,
        "[施設管理システム] 20██/5/12 通信喪失を検知。プロトコルF-7を発動。外周封鎖──完了。解除条件: ██████████",
    ),
    Clue(
        "p4_05", "散乱した調査装備", 4,
    ...
    Clue(
        "p4_06", "走査装置の動作ログ", 4,
        "[自動走査ログ] 設置場所: 区画間認証機構 20██/5/10 走査実行──非該当 20██/5/11 走査実行──非該当 20██/5/12 走査実行──該当(1) ──区画間移動を検出──",
    ),
    Clue(
    ...
    Clue(
        "p6_01", "逃走記録", 6,
        "[施設保安記録] 19██/██/██ 収容区画より逸脱1件。追跡チームを展開。確保に至らず。捜索を打ち切る。──対象の所在は現在も不明。",
    ),
    Clue(
        "p6_02", "封印装置", 6,
